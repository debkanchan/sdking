 ...
 */
function isHttpMethod(method: string): method is 'get' | 'post' | 'put' | 'delete' | 'options' | 'head' | 'patch' | 'trace' {
 ...
 */
export function generateRouteFiles(routes: RouteInfo[], outputDir: string, importPrefix: ".js" | ".ts" | false, writeFile: FileWriter, mkdir: DirMaker): void {
  ...
  for (const [routePath, pathRoutes] of routesByPath.entries()) {
    ...
    const routeCode = generateRouteFile(routePath, pathRoutes, routesByPath, importPrefix);
 ...
 */
function findDirectChildRoutes(parentPath: string, allRoutes: Map<string, RouteInfo[]>): string[] {
 ...
 */
function generateRoutesIndex(routesByPath: Map<string, RouteInfo[]>, outputDir: string, importPrefix: ".js" | ".ts" | false, writeFile: FileWriter): void {
  ...
  const topLevelRoutes = Array.from(routesByPath.keys())
  ...
  for (const routePath of topLevelRoutes) {
    ...
    const exportName = segment.startsWith('{') && segment.endsWith('}') 
      ? '$' + segment.substring(1, segment.length - 1) 
      : segment;
    
    imports.push(`import { routes as ${exportName}Routes } from './${exportName}/index${importPrefix ?? ''}';`);
 ...
 */
export function collectOperationIds(routes: RouteInfo[]): Array<{operationId: string, path: string, method: string}> {
  const operationIds: Array<{operationId: string, path: string, method: string}> = [];
 ...
 */
export function generateAliasFile(routes: RouteInfo[], outputDir: string, importPrefix: ".js" | ".ts" | false, writeFile: FileWriter): void {
  ...
  for (const { operationId, path, method } of operationIdMappings) {
    ...
    if (!importedPaths.has(routePath)) {
      imports.push(`import * as ${routeVarName} from './${routePath.replace('{', '$').replace('}', '')}/index${importPrefix ?? ''}';`);
      importedPaths.add(routePath);
    }
    
    exports.push(`${operationId}: ${routeVarName}.${method === 'delete' ? 'del' : method},`);
  }
 ...
 */
function generateRouteFile(routePath: string, routes: RouteInfo[], allRoutes: Map<string, RouteInfo[]>, importPrefix: ".js" | ".ts" | false): string {
  const imports = [];
  ...
  for (const route of routes) {
    ...
    if (route.requestBody?.content) {
      for (const contentType in route.requestBody.content) {
        ...
        findSchemaRefs(schema).forEach(schemaName => schemaImports.add(schemaName));
      }
    }
      ...
      for (const statusCode in route.responses) {
        ...
        if (response.content) {
          for (const contentType in response.content) {
            ...
            const method = route.method === 'delete' ? 'del' : route.method.toLowerCase();
            ...
            references.forEach(ref => {referencesContainsSelf = referencesContainsSelf || schemaCode === `${ref}Schema`});
  ...
  const relativeImportDepthFromRoot = Array(routePath.split('/').length).fill('..').join('/');
  
  // Add schema imports
  if (schemaImports.size > 0) {
    imports.push(`import { ${ Array.from(schemaImports).map(s => `${s}, ${s}Schema`).join(', ')} } from "${relativeImportDepthFromRoot}/schemas/index${importPrefix ?? ''}";`);
  }
  
  // Add the SDK configuration import
  imports.push(`import { sdkConfig } from "${relativeImportDepthFromRoot}/config${importPrefix ?? ''}";`);
  ...
  for (const childPath of childRoutes) {
    ...
    childRouteImports.push(`import { routes as ${importName}Routes } from '${importPath}/index${importPrefix ?? ''}';`);
  }
  ...
  for (const route of routes) {
    const functionName = route.method === 'delete' ? 'del' : route.method.toLowerCase();
 ...
 */
function generateRouteFunction(route: RouteInfo, routePath: string, functionName: string): string {
  ...
  // Build function body
  const functionBody = buildFunctionBody(route, routePath, pathParams, queryParams, hasRequestBody, responseType);
...
function getTypeForParameter(param: any): string {
  ...
  if (param.schema?.type === 'array') {
    if (param.schema.items?.$ref) {
      const itemRefName = param.schema.items.$ref.replace('#/components/schemas/', '');
      return `${itemRefName}[]`;
    }
...
function buildFunctionBody(
  ...
  if (pathParams.length > 0) {
    urlParts.push(`  let url = \`\${sdkConfig.baseUrl}${routePath.replace(/{([^}]+)}/g, '${pathParams.$1}')}\`;`);
  } else {
  ...
  if (queryParams.length > 0) {
    ...
    urlParts.push('    ' + queryParams.map(p => {
  ...
  responseParts.push('    throw new Error(`HTTP error ${response.status}: ${response.statusText}`);');
  responseParts.push('  }');
  responseParts.push('\n  // Parse the response');
