 ...
 */
export function generateSchemaFiles(schemas: SchemaInfo[], outputDir: string, importPrefix: ".js" | ".ts" | false, writeFile: FileWriter, mkdir: DirMaker): void {
  ...
  for (const schemaInfo of schemas) {
    ...
    const schemaCode = generateSchemaFile(schemaInfo.name, schemaInfo.schema, importPrefix);
 ...
 */
function generateSchemasIndex(schemas: SchemaInfo[], outputDir: string, importPrefix: ".js" | ".ts" | false, writeFile: FileWriter): void {
 ...
 */
export function generateSchemaFile(name: string, schema: SchemaObject, importPrefix: ".js" | ".ts" | false,): string {
  ...
  for (const ref of references) {
      imports.push(`import { ${ref}Schema } from "./${ref.toLowerCase()}${importPrefix ?? ''}";`);
  }
...
export function findSchemaRefs(schema: SchemaObject): string[] {
  ...
  function traverse(obj: SchemaObject): void {
    ...
    if (obj.type == "object" && obj.properties) {
      ...
      if (obj.additionalProperties && typeof obj.additionalProperties !== 'boolean') {
...
export function generateZodSchema(schema: SchemaObject): {
  ...
  if (schema.type === 'object') {
    ...
    const propEntries = Object.entries(properties).map(([propName, propSchema]) => {
      const {schemaCode: propZod, references: refs} = generateZodSchema(propSchema);
    ...
    if (schema.additionalProperties === true) {
      ...
      const {schemaCode: additionalPropertiesZod, references: refs} = generateZodSchema(schema.additionalProperties);
      catchAllProp = `.catchall(${additionalPropertiesZod})`;

      refs.forEach(ref => references.add(ref));
    }
    
    return {schemaCode: `z.object({\n${propEntries.join(',\n')}\n})${!!catchAllProp ? catchAllProp : ''}${schema.nullable ? '.nullable()' : ''}`, references: Array.from(references)};
  }
  
  if (schema.type === 'array') {
    // Handle array type
    const {schemaCode: itemsZod, references} = schema.items ? generateZodSchema(schema.items) : {schemaCode: 'z.any()', references: []};
    return {schemaCode: `z.array(${itemsZod})${schema.nullable ? '.nullable()' : ''}`, references};
  }
  
  if (schema.type === 'enum') {
    // Handle enum type
    const enumValues = schema.enum.map(value => typeof value === 'string' ? `"${value}"` : value);
    return {schemaCode: `z.enum([${enumValues.join(', ')}])${schema.nullable ? '.nullable()' : ''}`, references: []};
  }
  ...
  if (schema.type === 'allOf') {
    ...
    return {schemaCode: `z.intersection([${intersectionSchemas.join(', ')}])${schema.nullable ? '.nullable()' : ''}`, references: Array.from(references)};
  }
  ...
  if (schema.type === 'oneOf') {
    ...
    return {schemaCode: `z.union([${oneOfSchemas.join(', ')}])${schema.nullable ? '.nullable()' : ''}`, references: Array.from(references)};
  }
  ...
  if (schema.type === 'anyOf') {
    ...
    return {schemaCode: `z.union([${anyOfZod.join(', ')}])${schema.nullable ? '.nullable()' : ''}`, references: Array.from(references)};
  }
  ...
  switch (schema.type) {
    case 'string':
      ...
      return {schemaCode: `${stringSchema}${schema.nullable ? '.nullable()' : ''}`, references: []};
      
    case 'number':
    case 'integer':
      return {schemaCode: `z.number()${schema.nullable ? '.nullable()' : ''}`, references: []};
      
    case 'boolean':
      return {schemaCode: `z.boolean()${schema.nullable ? '.nullable()' : ''}`, references: []};
